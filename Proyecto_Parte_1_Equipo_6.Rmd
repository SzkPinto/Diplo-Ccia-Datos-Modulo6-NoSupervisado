...
```{r}
data<-read.csv("Sleep_health_and_lifestyle.csv")
...
write_xlsx(base_pca, "base_pca.xlsx")
...
```{r}
fviz_eig(pca_fit, addlabels = TRUE, ylim = c(0, 100))
...
El análisis de componentes principales pemitió reducir la dimensionalidad del conjunto de datos q < p, siendo q la nueva dimensión conservando la mayor cantidad de varianza original. Los 2 componentes principales explicaron el **73.16%** de variabilidad total, lo cual nos ayuda a representar en una gráfica de 2D ó R2, al mismo tiempo, los últimos componentes, los últimos 3 representan al rededor del 13 %, por lo que se puede prescindir de ellos.

Según la regla del codo, los tres primeros componentes explican el 86.4% de la variabilidad total, lo que sugiere que podrían ser suficientes para representar la estructura latente del fenómeno. Sin embargo, para fines de visualización y simplicidad, dos componentes ya ofrecen una representación robusta (73.16%).

La componente principal 1 (PC1) está influenciada por:
  - Age (0.39)
  - Occupation_Nurse (0.36)
  - Gender_Male (-0.35)
  - BMI_Category_Overweight (0.30)
  - Occupation_Doctor (-0.27)

Esto indica que PC1 captura un eje relacionado con características demográficas y ocupacionales, donde la edad y el sobrepeso (BMI) tienen una influencia positiva, mientras que ser hombre o médico se asocia negativamente.

Mientras que la componente principal 2 (PC2) está influenciada por:

  - Heart.Rate (0.43)
  - Stress.Level (0.38)
  - Sleep.Duration (-0.35)
  - Occupation_Engineer (-0.33)
La carga negativa de Sleep.Duration y positiva de Stress.Level en PC2 sugiere una relación inversa entre ambas variables, lo cual es coherente con literatura clínica: mayor estrés suele asociarse con menor duración del sueño.

Mientras que la componente principal 3 (PC3) está influenciada por:

  - Physical.Activity.Level	(-0.36)
  - Daily.Steps	(-0.39)
  - Occupation_Teacher	(0.32)
  - Blood.Pressure_X135.90	(0.30)
  
Posiblemente los pasos diarios tiene multicolonealidad con actividad física, por lo que podríamos prescindir de una, así como podría ser que la duración del sueño este correlacionada con el nivel de estrés.

Al final este ejercicio de PCA nos ayudó a no solo reducir la dimensionalidad del fenómeno, sino a identificar patrones latentes que vinvulan características fiaiológicas de cada individuo, ocupaciones y conductuales del transtorno del sueño, como actividad fisica y la presión sanguínea, nivel de estres y dración del sueño o el peso conforme a la edad.Lo cual si lo llevaríamos al ML, ya podríamos solo enfocarnos en 2 o 3 componentes principales que expliquen entre el 70 y 80% del fenómeno.



### 4.2.2 Modelo de tSNE

Se genera recipe para procesar las variables categóricas, se escalan y se centran los datos.

```{r}
rec <- recipe(~ ., data = df) %>%
  step_rm(Sleep.Disorder) %>%                # quitar target
  step_dummy(all_nominal_predictors()) %>%   # procesamiento variables categóricas
  step_center(all_numeric_predictors()) %>%  # centrar
  step_scale(all_numeric_predictors())       # escalar

```

Se preparan los features y se define la variable objetivo Sleep.Disorder

```{r}
#Preparación con recipe
prep_rec <- prep(rec)
features_processed <- bake(prep_rec, new_data = NULL)
```

```{r}
objetivo <- df$Sleep.Disorder
objetivo
```

Se aplica el algoritmo Rtsne con valores fijos de perplexity, eta y max_iter y se visualiza la formación de grupos bajo estos parámetros.

```{r}
#TSNE
set.seed(123)
tsne_res <- Rtsne(
  X = as.matrix(features_processed),
  perplexity = 30,
  eta = 200,
  max_iter = 1000,
...
df_tsne <- data.frame(
  Dim1 = tsne_res$Y[,1],
  Dim2 = tsne_res$Y[,2],
  Sleep.Disorder = objetivo
)

# VISUALIZAR
ggplot(df_tsne, aes(x=Dim1, y=Dim2, color=Sleep.Disorder)) +
  geom_point(size=1.5, alpha=0.7) +
  theme_minimal() +
  labs(title="t-SNE con recipe (Sleep dataset)",
       subtitle="Perplexity=30, eta=200, iter=1000")
```

Aquí se observa una separación clara entre los individuos con Sleep Apnea (azules) frente a los demás. Están agrupados en un clúster bastante definido hacia la derecha.
Los casos con None (verde) tienden a dispersarse en varios subgrupos, algunos bien compactos y otros más mezclados con los de Insomnia, por otro lado los de Insomnia (rojo) aparecen intercalados con los de None, lo que indica que estas dos clases pueden compartir características similares.

t-SNE logra encontrar patrones claros para Sleep Apnea, pero no tanto entre Insomnia y None, que parecen solaparse.

Con el fin de encontrar los parámetros que muestren la mejor reducción de funcionalidad se genera una rejilla con distintos valores de perplexity.
...
set.seed(123)
tsne_params <- expand.grid(perplexity = c(10, 15, 25, 30, 50))
...
p1
```
En este grafico vemos lo siguiente en estos rangos
Perplexity 10–15: El embedding genera estructuras muy fragmentadas, con clústeres más pequeños pero menos estables.
Perplexity 25–30: Se logra un mejor balance, donde se distingue bastante bien el grupo azul (Sleep Apnea) y se mantienen agrupamientos más consistentes en los verdes (None).
Perplexity 50: Se observa un "aplanamiento" de los grupos; los clústeres pierden definición y se dispersan más.
En general podemos decir que valores intermedios de perplexity (25–30) parecen dar representaciones más útiles y coherentes. Muy bajos (10) generan demasiado ruido, y muy altos (50) diluyen las separaciones.

Ahora la rejilla se genera variando los valores tanto de perplexity y eta.

```{r}
set.seed(123)
tsne_params2 <- expand.grid(
 perplexity = c(10, 20, 30),
 eta = c(50, 100, 200)
)
tsne_res2 <- lapply(seq(nrow(tsne_params2)), function(i) {
 Rtsne(
   X = as.matrix(features_processed),
   max_iter = 500,
   perplexity = tsne_params2$perplexity[i],
   eta = tsne_params2$eta[i],
   check_duplicates = FALSE,
   verbose = FALSE,
   pca = FALSE
 )
})
d2 <- rbindlist(lapply(seq(nrow(tsne_params2)), function(i) {
 data.table(
   x = tsne_res2[[i]]$Y[,1],
   y = tsne_res2[[i]]$Y[,2],
   perplexity = tsne_params2$perplexity[i],
...
*El dataset tiene patrones detectables: Sleep Apnea se distingue bien, mientras que Insomnia y None muestran solapamiento, lo que sugiere similitud en sus variables.
*El preprocesamiento con recipe (centrar, escalar, dummies) ayudó a que el embedding sea más estable.
*Los parámetros óptimos para este caso parecen ser perplexity 25–30 y eta=200.
