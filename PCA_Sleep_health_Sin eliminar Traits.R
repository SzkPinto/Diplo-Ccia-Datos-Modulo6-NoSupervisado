---
  title: "PCA_Sleep_health_and_lifestyle"
---
...
library(tidymodels) #installation para instalarlo
library(tidyverse)
library(knitr)
library(kableExtra)
library(GGally)
...
data<-read.csv("C:/Users/reyna.pinto/Documents/Suzuky/Diplomado Ccia DATOS UNAM/Modulo 6_Ciencia de DATOS_Diplomado UNAM. No supervisada/Sleep_health_and_lifestyle.csv")
...
str(data_noSleep)
## convertir a factor todas kas var dummies o categóricas antes de la matriz mixta
...
cor_matrix$correlations
 ...
 # visualizamos las correlaciones MIXTAS en un gráfico heatmap
...
# Mediante los análisis de correlación pudimos detectar que hay variables muy correlacionadas
# que prácticamente estarían aportando la misma información al PCA. Se puede optar por eliminar una de ellas
# para evitar sobreestimar la relevancia de las mismas, O promediarlas o INCLUIRLAS TODAS. Usaremos la primera opción.
...
# Nos quedaremos CON TODAS LAS VARIABLES PARA EL PCA (no hay descarte de var correlacionadas)
...
library(recipes)
base_recipe <- 
  recipe(Sleep.Disorder ~ ., data = data) %>%
...
# Mejorar la visualización reduciendo el número de componentes mostrados
        library(forcats)
base_pca %>%
  filter(component %in% paste0("PC", 1:8)) %>%  #desplegar solo los 6 primeros componentes
...
# De la misma forma podemos explorar la contribución de las traits a los compontentes 3.4.5.6.7.8
